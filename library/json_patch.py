...
class PatchManager(object):
    ...
    def write(self):
        ...
        self.module.atomic_move(
            tmpfile,
            to_native(
                os.path.realpath(to_bytes(self.outfile, errors="surrogate_or_strict")),
...
class JSONPatcher(object):
    ...
    def patch(self):
        ...
        for patch in self.operations:
            ...
            if (
                changed is not None or op == "remove"
            ):  # 'remove' will fail if we don't actually remove anything
                modified = bool(changed)
                if modified is True:
                    self.obj = new_obj
            if tested is not None:
                test_result = (
                    False if test_result is False else tested
        ...
        for idx, elem in enumerate(elements):
            try:
                ...
                if idx == (len(elements) - 1):  # this helps us stay idempotent
                    return None
                raise PathError(
                    "'%s' was not found in the JSON object" % path
            ...
            if isinstance(obj, dict):
                ...
                if path == "-":  # points to end of list
                    ...
                    if idx > len(obj):  # violation of rfc 6902
                        raise PathError(
                            "specified index '%s' cannot be greater than the number of elements in JSON array"
                            % path
                        )
    ...
    def replace(self, path, value, obj, **discard):
        ...
        if (
            old_value is None
        ):  # the target location must exist for operation to be successful
            raise PathError("could not find '%s' member in JSON object" % path)
